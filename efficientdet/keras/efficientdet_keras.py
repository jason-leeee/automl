...
class SOLOv2Head(tf.keras.layers.Layer):
  """SOLO-V2 head"""
  # seg_feat_channels=256, in_out_channels=64
  def __init__(self,
                num_classes,
                strategy,
                data_format,
  ...
  def call_single(self, ins_kernel_feat, idx, eval=False):
    ...
    kernel_feat = tf.image.resize(kernel_feat, size=[seg_num_grid, seg_num_grid], method='bilinear')
  ...
  def get_seg_single(self,
    ...
    inds = tf.where(inds!=0)  
    cate_labels = inds[:, 1]
    kernel_preds = kernel_preds[inds[:, 0]]

    # trans vector.
    # possible bug happens here
    size_trans = tf.identity(self.seg_num_grids)
    size_trans = tf.math.cumsum(tf.math.pow(size_trans, 2), 0)
    strides = tf.ones(size_trans[-1]) 
    ...
    kernel_preds = tf.reshape(kernel_preds, [I, N, 1, 1])
    seg_preds = tf.squeeze(tf.nn.conv2d(seg_preds, kernel_preds, strides=1), 0)
    seg_preds = tf.keras.activations.sigmoid(seg_preds)
    # mask.
    seg_masks = seg_preds > cfg.mask_thr
    sum_masks = tf.reduce_sum(seg_masks, [1,2]).float()
    ...
    # mask scoring.
    seg_scores = tf.reduce_sum(seg_preds * seg_masks.float(), [1,2]) / sum_masks
    ...
    seg_preds = tf.image.resize(tf.expand_dims(seg_preds, 0),
                                size=upsampled_size_out)[:, :, :h, :w]
    seg_masks = tf.squeeze(tf.image.resize(seg_preds,
                                size=ori_shape[:2]), 0)
  ...
  def get_seg(self, cate_preds, kernel_preds, seg_pred, img_metas, cfg, rescale=None):
    ...
    result_list = []
    # possible bug: other detach operations in pytorch may be missed 
    for img_id in range(len(img_metas)):
      cate_pred_list = [
          tf.stop_gradient(tf.reshape(cate_preds[i][img_id], [-1, self.cate_out_channels])) 
          for i in range(num_levels)
      ]
      seg_pred_list = tf.expand_dims(seg_pred[img_id, ...], 0)
      kernel_pred_list = [
          tf.stop_gradient(
            tf.reshape(tf.transpose(kernel_preds[i][img_id], perm=[1, 2, 0]), [-1, self.kernel_out_channels]))
            for i in range(num_levels)
      ]
      # this can be loaded from the tensorflow config: need check!
      img_shape = img_metas[img_id]['img_shape']
      scale_factor = img_metas[img_id]['scale_factor']
      ori_shape = img_metas[img_id]['ori_shape']

      cate_pred_list = tf.stack(cate_pred_list, 0)
      kernel_pred_list = tf.stack(kernel_pred_list, 0)
